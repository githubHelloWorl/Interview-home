<template>
	<div class="diy_details page_popular_jobs" id="popular_jobs_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_popular_jobs'>
							<div_popular_jobs :obj="obj"></div_popular_jobs>
...
<script>
	import div_popular_jobs from "@/components/diy/div_popular_jobs.vue";
	...
	export default {
		...
		data(){
			return{
				url_get_obj: "~/api/popular_jobs/get_obj?",

				field: "popular_jobs_id",
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/popular_jobs/set?popular_jobs_id=' + obj["popular_jobs_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.popular_jobs_id;
				}
			},
			/**
			 * 初始化收藏状态
