		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐热门工作模块(开始) -->
		<div class="warp" v-if="$check_action('/popular_jobs/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/popular_jobs/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_popular_jobs = !show_list_popular_jobs">
		                        <div class="switch_box">
		                            <span v-if="show_list_popular_jobs"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 热门工作推荐列表组件 -->
		                <list_popular_jobs v-if="show_list_popular_jobs" :list="list_popular_jobs" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_popular_jobs.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_popular_jobs" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_popular_jobs" :key="i" @click="to_details('popular_jobs',o,'popular_jobs_id')">
		                            <td v-for="(oj,n) in fields_popular_jobs">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	import mixin from "@/mixins/page.js";
				import list_popular_jobs from "@/components/diy/list_popular_jobs.vue";
	...
	export default {
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
						...
						// 获取热门工作列表
			get_popular_jobs() {
				let url = "~/api/popular_jobs/get_list?";
								// url = "~/api/popular_jobs/get_list?orderby=hits desc";
								this.$get(url, {
									"orderby": "hits desc",
							"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_popular_jobs = json.result.list;
					}
				})
			},
		...
		computed: {
					    list_table_experience_sharing() {
		        var list = this.list_popular_jobs;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_popular_jobs.map((o) => {
		                // 第二个中括号是对象的属性
