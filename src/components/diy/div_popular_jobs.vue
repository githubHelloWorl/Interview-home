<template>
	<div class="diy_details_box diy_div_popular_jobs">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/submit_resume/edit','add')" @click="to_form('/submit_resume/edit')" > 投递简历 </button>
											<button class="btn btn-primary details_btn" v-if="$check_action('/popular_jobs/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/popular_jobs/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "popular_jobs_id",
					source_id: this.obj["popular_jobs_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/popular_jobs/set?popular_jobs_id=' + _this.obj["popular_jobs_id"], {
								praise_len
							}, (res) => {
								if(res.result){
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.enterprise_name
																			query.img = this.obj.enterprise_logo
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var popular_jobs_id = this.obj.popular_jobs_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "popular_jobs",
          source_field: "popular_jobs_id",
          source_id: popular_jobs_id
        }, (res) => {
